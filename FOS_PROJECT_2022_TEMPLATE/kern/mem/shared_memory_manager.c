...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	for(int i=0;i<MAX_SHARES;i++)
	{
		if(shares[i].empty!=1)
		{
			...
			int x=strcmp(shares[i].name,shareName);
			if(x==0)
			{
				return 1000;
			}
	...
	struct Share* MyObject;
	int IndexOfMyObject;
	IndexOfMyObject=allocate_share_object(&MyObject);
	if(IndexOfMyObject==E_NO_SHARE)
	{

		return 1000;
	}
allocate_chunk(myenv->env_page_directory,(uint32)virtual_address,(uint32)size,PERM_WRITEABLE|PERM_USER);

	MyObject->ownerID=ownerID;
	for(int y=0;y<64;y++)
	{
		MyObject->name[y]=shareName[y];
	}
	MyObject->size=size;
	MyObject->references=1;
    MyObject->isWritable=isWritable;
	uint32 index=0;
	for(int i=0;i<size;i+=PAGE_SIZE)
	{
		uint32* ptrpage;
		struct FrameInfo* frame;
		frame=get_frame_info(myenv->env_page_directory,(uint32)(virtual_address+i),&ptrpage);
		add_frame_to_storage(MyObject->framesStorage,frame,index);
		index++;


	}
	return IndexOfMyObject;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	////////////
	int indexOfObject;
	indexOfObject=get_share_object_ID(ownerID,shareName);
	if(indexOfObject==E_SHARED_MEM_NOT_EXISTS)
	{
	    return 1000;
	}
	shares[indexOfObject];
	int index=0;
	for(int i=0;i<shares[indexOfObject].size;i+=PAGE_SIZE)
	{  struct FrameInfo* frame;
	///////////////////////////
//	uint32* PageTablePtr;
//	get_page_table(myenv->env_page_directory,(uint32)(virtual_address+i),&PageTablePtr);
//				if(PageTablePtr==NULL)
//				{
//					create_page_table(myenv->env_page_directory,(uint32)(virtual_address+i));
//				}
	//////////////////////

	    frame=get_frame_from_storage(shares[indexOfObject].framesStorage,index);

	    if(shares[indexOfObject].isWritable==1)
	    {
	    	map_frame(myenv->env_page_directory,frame,(uint32)(virtual_address+i),PERM_WRITEABLE|PERM_USER);
	    	index++;
	    ...
	    else
	    {
	    	...
	    	map_frame(myenv->env_page_directory,frame,(uint32)(virtual_address+i),PERM_USER);
	    	index++;
	    }
	...
	shares[indexOfObject].references++;
...
/////////////////////
	...
	free_user_mem(myenv,(uint32)startVA,shares[sharedObjectID].size);
	shares[sharedObjectID].references=shares[sharedObjectID].references-1;
	if(shares[sharedObjectID].references==0)
	{
		int x=free_share_object(sharedObjectID);
	...
	tlbflush();
	//tlb_invalidate((void*)myenv->env_page_directory,(void*)startVA);
	return 0;
